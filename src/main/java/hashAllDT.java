...
//		Location_training = Location_training.union(Location_training3).un.distinct();
		...
		Dataset<Row> Location_explode = Location_afterStop.withColumn("split",org.apache.spark.sql.functions.explode(Location_afterStop.col("words2")));
		Dataset<Row> Location_temp = Location_explode.groupBy("split").agg(org.apache.spark.sql.functions.collect_list("split").as("array"));

		Location_afterStop = Location_temp;
		int numFeatures = 5;
		...
		Location_vector = Location_vector.select(Location_vector.col("array").as("Words"),Location_vector.col("rawFeatures"),Location_vector.col("features"),Location_vector.col("Label").as("label"));
...
//		System.out.println(Merchant_afterStop.count());

		Dataset<Row> Merchant_explode = Merchant_afterStop.withColumn("split",org.apache.spark.sql.functions.explode(Merchant_afterStop.col("words2")));
//		Merchant_explode.show(false);
		Dataset<Row> Merchant_temp = Merchant_explode.groupBy("split").agg(org.apache.spark.sql.functions.collect_list("split").as("array")).distinct();
		Merchant_temp.show(false);
		Merchant_afterStop = Merchant_temp;
		System.out.println(Merchant_afterStop.count());
		...
		Merchant_vector = Merchant_vector.select(Merchant_vector.col("array").as("Words"),Merchant_vector.col("rawFeatures"),Merchant_vector.col("features"),Merchant_vector.col("Label").as("label"));
...
//		Dataset<Row> validationDF = Location_validationDF.union(Merchant_validationDF);
		...
		DecisionTreeClassifier dt = new DecisionTreeClassifier()
				  ...
				  .setSeed(1L);
		DecisionTreeClassificationModel NewdtModel = dt.fit(trainingDF);

		/*NaiveBayes nb = new NaiveBayes();		
		NaiveBayesModel model = nb.fit(trainingDF);*/
		Dataset<Row> predictions = NewdtModel.transform(testingDF);
...
//		mergedDF.filter(mergedDF.col("label").equalTo(1.0)).show(250);
		...
		try {
			NewdtModel.write().overwrite().save("/Users/nishantarora/Downloads/myDecisionTreeClassificationModel2");
		} catch (IOException e) {
