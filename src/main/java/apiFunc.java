...
public class apiFunc {
	...
	// Loading the Spark Configuration
				public static SparkConf conf = new SparkConf().setAppName("Name").setMaster("local");
				public static JavaSparkContext sc = new JavaSparkContext(conf);
				public static SparkSession ssc = new SparkSession(sc.sc());
				public static SQLContext spark = ssc.sqlContext();
	
	public static List<NameEntity> stringPredictor(String x) {
			...
			for(Row r: predict.collectAsList()){
				...
				if(r.getAs(2).equals(0.0) && locFlag)
				{
					if(v.apply(0)>=0.7)
					{
						...
						ne.setCategory("location");
						ne.setName(loc);
						ne.setProbability(v.apply(0));
						nameEntities.add(ne);
				...
				else if(r.getAs(2).equals(1.0) && mercFlag)
				{
					if(v.apply(1)>=0.7)
					{
						...
						ne.setCategory("merchant");
						
						ne.setName(merc);
						ne.setProbability(v.apply(1));
						nameEntities.add(ne);
