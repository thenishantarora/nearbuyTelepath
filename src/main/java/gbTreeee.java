...
//		System.out.println(explode.select("one").count() + " " + Location_afterStop.select("words2").count());
		explode1 = explode1.withColumn("one",org.apache.spark.sql.functions.collect_list("one").as("uniArray"));
...
//		Location_afterStop.show(false);
		...
		Location_vector = Location_vector.select("words2","rawFeatures","features");
...
//		System.out.println(explode.select("one").count() + " " + Location_afterStop.select("words2").count());
		...
		Merchant_vector = Merchant_vector.select("words2","rawFeatures","features");
