...
//		Dataset<Row> validationDF = Location_validationDF.union(Merchant_validationDF);
		...
		LogisticRegression lr = new LogisticRegression().setMaxIter(20).setRegParam(0.05);
...
//		mergedDF.filter(mergedDF.col("label").equalTo(1.0)).show(250);
		try {
			lrModel.write().overwrite().save("/Users/nishantarora/Downloads/lrModel");
		} catch (IOException e) {
