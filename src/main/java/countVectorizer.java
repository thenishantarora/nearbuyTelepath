...
//		Combined_vector = Combined_vector.select(Combined_vector.col("grams").as("Words"),Combined_vector.col("label"));
		...
		CountVectorizerModel cvModel = new CountVectorizer().setInputCol("Words").setOutputCol("features").setMinDF(0).fit(Combined_vector);
		Combined_vector = cvModel.transform(Combined_vector);
		try {
//			model.write().overwrite().save("/Users/nishantarora/Downloads/w2vModel");
			cvModel.write().overwrite().save("/Users/nishantarora/Downloads/cvModel");
		...
		Dataset<Row> featurizedData = hashingTF.transform(Combined_vector);
				// alternatively, CountVectorizer can also be used to get term frequency vectors
...
//		Combined_vector = featurizedData;
		try {
//			model.write().overwrite().save("/Users/nishantarora/Downloads/w2vModel");
			hashingTF.write().overwrite().save("/Users/nishantarora/Downloads/hashModel");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
...
//		predictions.show(false);
		...
		try {
			gbtModel.write().overwrite().save("/Users/nishantarora/Downloads/countVectorizer");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
