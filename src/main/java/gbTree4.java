...
//		System.out.println(explode.select("one").count() + " " + Location_afterStop.select("words2").count());
		...
		System.out.println(Location_afterStop.count());
//		Word2Vec Location_word2Vec = new Word2Vec().setInputCol("uniArray").setOutputCol("w2v").setVectorSize(4).setMinCount(0);
//		Word2VecModel Location_model = Location_word2Vec.fit(Location_afterStop);
//		Dataset<Row> Location_w2v = Location_model.transform(Location_afterStop).select("uniArray","w2v");
//		explode.show(false);
//		Location_afterStop.show(false);
		...
		Location_vector = Location_vector.select("uniArray","rawFeatures","features");*/
		
		

		Location_vector = Location_vector.withColumn("Label", functions.lit(0.0));		
		Location_vector = Location_vector.select(Location_vector.col("uniArray").as("Words"),Location_vector.col("rawFeatures"),Location_vector.col("features"),Location_vector.col("Label").as("label"));
...
//		Word2Vec Merchant_word2Vec = new Word2Vec().setInputCol("uniArray").setOutputCol("w2v").setVectorSize(4).setMinCount(0);
//		Word2VecModel Merchant_model = Merchant_word2Vec.fit(Merchant_afterStop);
//		Dataset<Row> Merchant_w2v = Merchant_model.transform(Merchant_afterStop).select("uniArray","w2v");
		...
		Merchant_vector = Merchant_vector.select("uniArray","rawFeatures","features");*/

		Merchant_vector = Merchant_vector.withColumn("Label", functions.lit(1.0));		
		Merchant_vector = Merchant_vector.select(Merchant_vector.col("uniArray").as("Words"),Merchant_vector.col("rawFeatures"),Merchant_vector.col("features"),Merchant_vector.col("Label").as("label"));
...
//		predictions.show(false);
		...
		try {
			gbtModel.write().overwrite().save("/Users/nishantarora/Downloads/gbTreeee");
		} catch (IOException e) {
