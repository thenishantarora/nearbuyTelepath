...
//		Combined_vector = Combined_vector.select(Combined_vector.col("uni").as("Words"),Combined_vector.col("label"));
		...
		Word2Vec word2Vec = new Word2Vec().setInputCol("Words").setOutputCol("features").setVectorSize(10).setMinCount(0).setWindowSize(1);
		Word2VecModel model = word2Vec.fit(Combined_vector);
		Dataset<Row> w2v = model.transform(Combined_vector).select("Words","features","label");
		w2v.show(false);
		try {
...
//		predictions.show(false);
		...
		try {
			gbtModel.write().overwrite().save("/Users/nishantarora/Downloads/w2vFinal");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
