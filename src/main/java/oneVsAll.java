...
//		GBTClassificationModel gbtModel = gbt.fit(trainingDF);
		...
		OneVsRest ovr = new OneVsRest().setClassifier(dt);
...
//		RandomForestClassificationModel forestModel  = forest.fit(trainingDF);
		...
		Dataset<Row> predictions = ovrModel.transform(testingDF);
//		predictions.show(false);
		...
		predictions.show(false);
		try {
			ovrModel.write().overwrite().save("/Users/nishantarora/Downloads/oneVsAll");
		} catch (IOException e) {
